...
The response we get back from this call is of type `IBulkResponse`.
This interface exposes an `Errors` boolean property, indicating
whether or not any errors were generated as part of the bulk
operation.  In addition, the response exposes both an `Items` of type
`IEnumerable<BulkOperationResponseItem>`, containing details about
each item indexed as part of the request.  It also exposes an
`ItemsWithErrors` property of the same type.  We can inspect the
response as follows:
...
If all the docs are going to be added to the same index, we can use a
shortcut to avoid having to specify the index for each
`BulkIndexDescriptor`.  When setting up the `ConnectionSettings` of
our client, we can use the `SetDefaultIndex` method to specify an
...
After doing this, we can simplify our calls as follows:
...
Finally, if we have a collection of documents that we want to bulk index, there's
another approach we can take that works nicely when iterating through the collection.

The `Bulk` function is overloaded to also accept an object that implements the `IBulkRequest` interface.
The `BulkDescriptor` type implements this interface, so we can instantiate one of those, apply all the
documents in our collection to that object, then pass that to the `Bulk` method to index our documents:
